...
const create = async ({ userName, email, password_hash, roles }) => {
    ...
    try{
        const id = await accountRepository.create({ userName, email, password_hash, roles });
        return getById(id);
    }
    catch(error){
...
const register = async ( {userName, email, password} ) => {
    try {
        ...
        const accountId = await accountRepository.create({userName, email, passwordHash, roles: [Role.USER] });
